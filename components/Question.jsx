...
function Question({ question }) {
    const handleClick = () => {
            router.push({
              ...
              query: { _id: question.id },
            })
    }
    return (
        <div className="question max-w-xs min-h-[175px] mx-auto p-4 rounded-lg bg-white flex flex-col transition-sm hover:scale-[1.01]" onClick={handleClick}>
            <div className="flex space-x-2 flex-col-reverse sm:flex-row">
                ...
                <img src={`/images/que/${question.category.toLowerCase()}.jfif`} layout="fill" className="w-10 h-10 border-4 border-white hover:scale-105 transition-md object-cover rounded-full -translate-x-3 sm:-translate-x-0" />
            </div>
