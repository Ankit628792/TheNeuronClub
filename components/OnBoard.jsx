...
function OnBoard(props) {
    ...
    return (
        <>
            {data?.[activeIndex] && data[activeIndex].imgSrc && <div className="h-screen top-0 left-0 absolute w-full z-50">
                <div className="max_w_3xl overflow-hidden fixed h-screen p-5 blur-black w-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                    {activeIndex > 0 && <button className="outline-none border-none focus:border-none focus:outline-none absolute h-1/2 top-1/2 -translate-y-1/2 left-0" onClick={() => setActiveIndex(activeIndex - 1)}><ArrowLeftIcon className="w-10 h-10 p-1 opacity-50 text-gray-800 mx-2 bg-white rounded-full shadow-lg" /> </button>}
                    <div className={`w-full h-full sm:p-10 md:p-20 max-h-screen flex ${activeIndex % 2 == 0 ? 'flex-col' : 'flex-col-reverse'} justify-center items-center`}>
                        <motion.img
                            ...
                            transition={{
                                ...
                                stiffness: 260,
                                damping: 50,
                            }} src={data[activeIndex].imgSrc} className="p-5 h-80 sm:h-full rounded-lg object-contain" alt="" />
                        <h1 className="p-5 text-center text-white text-xl max-w-xl blur-white">{data[activeIndex].text}</h1>
                    </div>
                    <button className="outline-none border-none focus:border-none focus:outline-none absolute h-1/2 top-1/2 -translate-y-1/2 right-0" onClick={() => activeIndex === data?.length - 1 ? props.onSelect() : setActiveIndex(activeIndex + 1)} ><ArrowRightIcon className="w-10 h-10 p-1 opacity-50 text-gray-800 mx-2 bg-white rounded-full shadow-lg" /> </button>
                </div>
