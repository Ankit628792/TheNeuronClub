...
function Portfolio() {
    ...
    return (
        <>
            ...
            <div className="flex flex-wrap sm:justify-center w-full gap-y-4 gap-x-6 md:gap-x-8 xl:gap-x-10 text-lg lg:text-xl mx-auto font-medium text-gray-700 px-10 pt-28 lg:pt-32 pb-5">
                ...
                <div className="flex items-center max-w-max">
                    <h1>Email:&nbsp; </h1>
                    <h2 className="font-normal text-lg break-all">{session?.email}</h2>
                </div>
                <div className="flex items-center max-w-max">
                    <h1>Username:&nbsp; </h1>
                    {isEdit ? <input type="text" name="username" onChange={handleChange} value={data.username} className="outline-none text-lg h-10 px-2 transition duration-200 bg-white border border-gray-300 rounded shadow-sm appearance-none focus:outline-none focus:shadow-outline" /> : <h2 className="font-normal text-lg break-all">{session?.username}</h2>}
                </div>
                <div className="flex items-center max-w-max">
                    <h1>Country:&nbsp; </h1>
                    {isEdit ? <select onChange={handleChange} className="outline-none text-lg h-10 cursor-pointer px-2 transition duration-200 bg-white border border-gray-300 rounded shadow-sm appearance-none focus:outline-none focus:shadow-outline" type="country" name="country" value={data.country} required placeholder="Country ">
                        ...
                        : <h2 className="font-normal text-lg break-all">{session.country}</h2>}
                </div>
                <div className="flex items-center max-w-max">
                    <h1>Balance:&nbsp; </h1>
                    <h2 className="inline-flex items-center font-normal text-lg"><Coin width="5" height="5" />{amount}</h2>
                </div>
                {/* <div className="flex items-center max-w-max">
                    <h1>Referred user:&nbsp; </h1>
            ...
            <div className={`p-5 py-10 sm:p-10 xl:px-20 min-w-full mx-auto`}>
                ...
                {userQuestions?.length > 0 &&
                    <>
                        <h1 className="text-2xl sm:text-3xl font-semibold  text-gray-700 my-6 sm:px-5 border-b">My Questions</h1>
