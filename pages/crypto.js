    ...
    return (
        <>
            <div className='text-white text-center max-w-5xl mx-auto p-5 my-10 min-h-[350px]'>
                ...
                {currencies && <div className="my-2 lg:my-4 flex justify-center items-center space-x-10">
                    <input
                    ...
                    >
                        {currencies?.map(item => <option key={item.uuid} value={item.uuid} className="capitalize">{item.symbol}</option>)}
                    </select>
                </div>}
